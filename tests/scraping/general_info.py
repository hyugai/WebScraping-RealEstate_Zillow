...
def scrape():
    ...
    results = general_homes_scraper.main(cities_hrefs[:6])
    print(len(results))

    #with sqlite3.connect(db_path) as conn:
    #    cur = conn.cursor()
    #    cur.execute("CREATE TABLE IF NOT EXISTS home (id INTEGER UNIQUE, info TEXT, extension TEXT DEFAULT NULL, is_extended INTEGER DEFAULT 0)")
